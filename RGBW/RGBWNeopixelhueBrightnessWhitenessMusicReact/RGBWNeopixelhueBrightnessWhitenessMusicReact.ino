...
void runNormalLoop(){  // this is the whole function of the previous SIMPLE RBGW POT sketch
  ...
  int potControlHue  = analogRead(2); // assign analog reading of left potentiometer knob to control Whiteness
  int potControlWhiteness  = analogRead(3);  // assign analog reading of right potentiometer knob to control hue
  ...
  else if (modeA==1) {brightness = map(potControlHueAverage2, 0, 1023, 0, 255);} //jpdate Brightness Values
  
  
    
   // map right pot knob to 0-255 values of hue
   saturation = 255;                               // saturation is a number between 0 - 255
   whiteness = map(potControlWhitenessAverage, 1024, 0, 0, 255); // map left pot knob to 0-255 values of Whiteness
