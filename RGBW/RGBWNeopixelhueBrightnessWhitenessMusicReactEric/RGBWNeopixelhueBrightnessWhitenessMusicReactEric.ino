...
#include "EEPROM.h"
#define PIN 6
#define CNT_LIGHTS 25 // Max LEDs is ~170 with RGBW array (more is unstable).
...
int potControlLeftAverage = 0;   // the average
int potControlRightAverage = 0;   // the average
...
int white;
int whiteness = 0; //starting value
...
void loop() {
detectButtonPress();
if (modeB==0){runNormalLoop();}         //run normal hue selection code
...
void runNormalLoop(){  // this is the whole function of the previous SIMPLE RBGW POT sketch
  ...
  int potControlLeft  = analogRead(2); // assign analog reading of left potentiometer knob to control Whiteness
  int potControlRight  = analogRead(3);  // assign analog reading of right potentiometer knob to control hue
//  int potControlWhiteness  = 10;
    
  // Code for Left Analog Pot smoothing
  // subtract the last reading:
  total = total - readings[readIndex];
  // read from the pot:
  readings[readIndex] = potControlLeft;
  ...
  potControlLeftAverage = total / numReadings;
  // send it to the computer as ASCII digits
  //Serial.println(potControlLeftAverage);
  delay(1);        // delay in between reads for stability
        
  // set HSB & White values


   // Code for Right Analog Pot smoothing
  // subtract the last reading:
  total2 = total2 - readings2[readIndex2];
  // read from the pot:
  readings2[readIndex2] = potControlRight;
  // add the reading to the total:
  total2 = total2 + readings2[readIndex2];
  // advance to the next position in the array:
  ...
  // calculate the average:
  potControlRightAverage = total2 / numReadings2;
  // send it to the computer as ASCII digits
  //Serial.println(potControlRightAverage);
  delay(1);        // delay in between reads for stability
  ...
  if (modeA==0){hue = map(potControlLeftAverage, 0, 1023, 0, 255);}   //update Left Values
  else if (modeA==1) {hue = map(potControlLeftAverage, 0, 1023, 0, 255);} //update Left Values
  
  
    
   // map right pot knob to 0-255 values of brightness
   saturation = 255;                               // saturation is a number between 0 - 255
   if(modeA==0){brightness = map(potControlRightAverage, 1024, 0, 0, 255);} // map left pot knob to 0-255 values of Whiteness
   else if (modeA==1){whiteness = map(potControlRightAverage, 1024, 0, 0, 255);}
...
void adjust_refresh_rate(int amplitude, int freq_band) {
  if( amplitude > 400 && freq_band <= 1 )
    //tmp_refresh_adj += 23;// Remove propogation speed increase on loud volume peaks
    tmp_refresh_adj += 0;
...
void set_pixel_colors(int cur_LED_vals[]) {
  ...
  for (i = 0; i < CNT_LIGHTS; i++) {
    ...
    if(cur_LED_vals[i] > 500) {
      ...
      pixels.setPixelColor( i, useColor[0], 0, useColor[1], useColor[2] );  //this just sets the white to ZERO
    } else {
      pixels.setPixelColor( i, 0, 0, 0, 0 );
    }//if
