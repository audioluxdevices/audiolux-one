...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(CNT_LIGHTS, PINforControl, NEO_RWGB + NEO_KHZ800);
...
void set_pixel_colors(int cur_LED_vals[]) {
  ...
  for (i = 0; i < CNT_LIGHTS; i++) {
    ...
    if(cur_LED_vals[i] > 500) {
      ...
      strip.setPixelColor( i, useColor[0], 0, useColor[1], useColor[2] );  //this just sets the white to ZERO
    } else {
      strip.setPixelColor( i, 0, 0, 0, 0 );
      //strip.setPixelColor(0, 100,100,100,100);
    }  
  }
}
